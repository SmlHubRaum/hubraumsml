...
class ENSHttpSession(object):
	...
	#request_url = request_url.replace("http","https");
	    ...
	    print "--------------------"
            ...
            response = requests.get(request_url, auth=HTTPBasicAuth('admin', 'Sml1Orchestrator'),headers = headers)
...
class ENSClient(object):
    ...
    def init(self):
	    ...
	    with open("Provision_request.txt", "w") as text_file:
		text_file.write("Provision-Request: {0}".format(url))
	    import os
	    i_m=0
	    while os.path.exists("Provision_request%s.txt" % i_m):
    		i_m += 1
	    with open("Provision_request%s.txt" % i_m, "w") as text_file:
		text_file.write("Provision-Request: {0}".format(url))
	    ...
	    #import os
	    i_m=0
	    while os.path.exists("provisining_backup%s.json" % i_m):
    		i_m += 1
	    with open('provisining_backup%s.json' % i_m, 'w') as outfile:
		json.dump(provision_response.json(), outfile)
    ...
    def close(self):
		...
		logging.info('mge: http delete: {}'.format(app_delete_url))
		    ...
		    file.close()
 		    ...
 		    file = open("Provision_request.txt", "w") 
		    file.write("")
		    file.close()
                else:
